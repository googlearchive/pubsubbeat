...
all references to old fields, you should set `migration.6_to_7.enabled: false`
so that field aliases will not be created during your next minor upgrade.
...
And the corresponding {es} index template is named +metricbeat-{stack-version}+.
...
deletes all data indexed into the metricbeat-{stack-version} indices on
2019.04.02.
